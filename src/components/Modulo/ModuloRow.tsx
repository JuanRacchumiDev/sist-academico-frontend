...
export const ModuloRow: React.FC<Props> = ({ modulo }) => {
  ...
  const actionText = modulo.estado ? "Desactivar" : "Activar";
  const ActionIcon = modulo.estado ? ToggleLeft : ToggleRight;
  const actionColor = modulo.estado ? "text-red-600" : "text-green-600";
  const hoverBgColor = modulo.estado
    ? "hover:bg-red-100"
    : "hover:bg-green-100";
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={modulo.estado ? "text-red-500" : "text-green-500"}
          />
        }
      />
    </>
