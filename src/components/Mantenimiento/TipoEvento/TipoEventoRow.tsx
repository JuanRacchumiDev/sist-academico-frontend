...
interface Props {
  tipoEvento: DetalleParametro;
  onStatusChange?: (tipoEventoId: number) => void;
}

export const TipoEventoRow: React.FC<Props> = ({ tipoEvento }) => {
  ...
  const nuevoEstado = !tipoEvento.estado;
  ...
  const handleConfirmStatus = async () => {
    ...
    try {
      ...
      //     response as tipoEventoResponse;
      ...
      //     if (onStatusChange) {
      //       onStatusChange(tipoEvento.id);
      //     }
      //   } else {
      //     showToast("error", error || "Error al actualizar el tipoEvento.");
      //   }
  ...
  const actionText = tipoEvento.estado ? "Desactivar" : "Activar";
  const ActionIcon = tipoEvento.estado ? ToggleLeft : ToggleRight;
  const actionColor = tipoEvento.estado ? "text-red-600" : "text-green-600";
  const hoverBgColor = tipoEvento.estado
    ? "hover:bg-red-100"
    : "hover:bg-green-100";
  ...
  return (
    <>
      ...
      >
        <TableCell className="py-3">{tipoEvento.nombre}</TableCell>
        <TableCell className="py-3">{tipoEvento.descripcion}</TableCell>
          ...
          <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
            ...
            >
              ...
              >
                ...
                <span>{actionText} Tipo Evento</span>
              </DropdownMenuItem>
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={tipoEvento.estado ? "text-red-500" : "text-green-500"}
          />
        }
      />
    </>
