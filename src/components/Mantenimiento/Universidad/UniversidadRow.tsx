...
interface Props {
  universidad: DetalleParametro;
  onStatusChange?: (universidadId: number) => void;
}

export const UniversidadRow: React.FC<Props> = ({ universidad }) => {
  ...
  const nuevoEstado = !universidad.estado;
  ...
  const handleConfirmStatus = async () => {
    ...
    try {
      ...
      //     if (onStatusChange) {
      //       onStatusChange(universidad.id);
      //     }
      //   } else {
      //     showToast("error", error || "Error al actualizar el universidad.");
      //   }
  ...
  const actionText = universidad.estado ? "Desactivar" : "Activar";
  const ActionIcon = universidad.estado ? ToggleLeft : ToggleRight;
  const actionColor = universidad.estado ? "text-red-600" : "text-green-600";
  const hoverBgColor = universidad.estado
    ? "hover:bg-red-100"
    : "hover:bg-green-100";
  ...
  return (
    <>
      ...
      >
        <TableCell className="py-3">{universidad.nombre}</TableCell>
        <TableCell className="py-3">{universidad.abreviatura}</TableCell>
          ...
          <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
            ...
            >
              ...
              >
                ...
                <span>{actionText} Universidad</span>
              </DropdownMenuItem>
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={universidad.estado ? "text-red-500" : "text-green-500"}
          />
        }
      />
    </>
