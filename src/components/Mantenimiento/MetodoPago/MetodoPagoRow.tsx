...
interface Props {
  ...
  onStatusChange?: (metodoPagoId: number) => void;
}

export const MetodoPagoRow: React.FC<Props> = ({ metodoPago }) => {
  ...
  const nuevoEstado = !metodoPago.estado;
  ...
  const actionText = metodoPago.estado ? "Desactivar" : "Activar";
  const ActionIcon = metodoPago.estado ? ToggleLeft : ToggleRight;
  const actionColor = metodoPago.estado ? "text-red-600" : "text-green-600";
  const hoverBgColor = metodoPago.estado
    ? "hover:bg-red-100"
    : "hover:bg-green-100";
  ...
  return (
    <>
      ...
      >
        <TableCell className="py-3">{metodoPago.nombre}</TableCell>
        <TableCell className="py-3">{metodoPago.descripcion}</TableCell>
          ...
          <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
            ...
            >
              ...
              >
                ...
                <span>{actionText} Método Pago</span>
              </DropdownMenuItem>
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={metodoPago.estado ? "text-red-500" : "text-green-500"}
          />
        }
      />
    </>
