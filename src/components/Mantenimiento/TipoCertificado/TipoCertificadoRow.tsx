...
interface Props {
  ...
  onStatusChange?: (tipoCertificadoId: number) => void;
}

export const TipoCertificadoRow: React.FC<Props> = ({ tipoCertificado }) => {
  ...
  const nuevoEstado = !tipoCertificado.estado;
  ...
  const handleConfirmStatus = async () => {
    ...
    try {
      ...
      //     response as tipoCertificadoResponse;
      ...
      //     if (onStatusChange) {
      //       onStatusChange(tipoCertificado.id);
      //     }
      //   } else {
      //     showToast("error", error || "Error al actualizar el tipoCertificado.");
      //   }
  ...
  const actionText = tipoCertificado.estado ? "Desactivar" : "Activar";
  const ActionIcon = tipoCertificado.estado ? ToggleLeft : ToggleRight;
  const actionColor = tipoCertificado.estado
    ? "text-red-600"
    : "text-green-600";
  const hoverBgColor = tipoCertificado.estado
    ? "hover:bg-red-100"
    : "hover:bg-green-100";
  ...
  return (
    <>
      ...
      >
        <TableCell className="py-3">{tipoCertificado.nombre}</TableCell>
        <TableCell className="py-3">{tipoCertificado.descripcion}</TableCell>
        <TableCell className="py-3">
          {tipoCertificado.estado ? (
          ...
          <DropdownMenu open={isDropdownOpen} onOpenChange={setIsDropdownOpen}>
            ...
            >
              ...
              >
                ...
                <span>{actionText} Tipo Certificado</span>
              </DropdownMenuItem>
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={
              tipoCertificado.estado ? "text-red-500" : "text-green-500"
            }
          />
        }
      />
