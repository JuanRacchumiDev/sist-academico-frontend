...
export const EventoRow: React.FC<Props> = ({ evento }) => {
  ...
  const actionText = evento.estado ? "Desactivar" : "Activar";
  const ActionIcon = evento.estado ? ToggleLeft : ToggleRight;
  const actionColor = evento.estado ? "text-red-600" : "text-green-600";
  const hoverBgColor = evento.estado
    ? "hover:bg-red-100"
    : "hover:bg-green-100";
  ...
  return (
    <>
      ...
      >
        <TableCell className="py-3">{evento.titulo}</TableCell>
        <TableCell className="py-3">{evento.tipo_evento?.nombre}</TableCell>
        <TableCell className="py-3">
          {evento.categoria_evento?.nombre}
        </TableCell>
        <TableCell className="py-3">
          {evento.estado ? (
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={evento.estado ? "text-red-500" : "text-green-500"}
          />
        }
      />
    </>
