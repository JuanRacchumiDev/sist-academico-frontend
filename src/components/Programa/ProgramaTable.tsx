...
export const ProgramaTable = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await getProgramasPaginate(currentPage, limit, filters);

      const { result, data, pagination: newPagination } = response;

      if (result && data) {
        const dataProgramas = data as Programa[];
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-100">
              ...
              <TableHead className="text-gray-600 font-medium">Sigla</TableHead>
              <TableHead className="text-gray-600 font-medium">
                Nombre
              </TableHead>
              <TableHead className="text-gray-600 font-medium">
